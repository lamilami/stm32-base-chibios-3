...
#ifdef STM32F100C8
#define OW_USART_CLK	STM32_PCLK2
#else
...
#ifdef STM32F100C8
  OW_USART->SR = (uint16_t)~USART_SR_TC;
  //	USART_SendData(OW_USART, 0xf0);

  /* Transmit Data */
  OW_USART->DR = (0xf0 & (uint16_t) 0x01FF);
#else
...
uint8_t OW_Send(uint8_t sendReset, uint8_t *command, uint8_t cLen, uint8_t *data, uint8_t dLen, uint8_t readStart) {
  ...
  while (cLen > 0) {
    ...
    DMA1->IFCR |= ((uint32_t)(DMA_ISR_GIF5 | DMA_ISR_TCIF5 | DMA_ISR_HTIF5 | DMA_ISR_TEIF5));
    ...
    /* Write to DMAy Channelx CPAR */OW_DMA_CH_RX->CPAR = (uint32_t) & (OW_USART->RDR);
    ...
    /*--------------------------- DMAy Channelx CPAR Configuration ---------------*/
    /* Write to DMAy Channelx CPAR */OW_DMA_CH_TX->CPAR = (uint32_t) & (OW_USART->TDR);
