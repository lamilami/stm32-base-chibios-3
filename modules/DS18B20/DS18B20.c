...
//		OW_Send(OW_SEND_RESET, "\xcc\xbe\xff\xff", 4, DS_OUT.buf, 2, 2);
		...
		if (OW_Send(OW_SEND_RESET,
				(uint8_t *) "\x55\x28\x08\x38\xbf\x03\x00\x00\xea\xbe\xff\xff",
				12, DS_OUT[0].buf, 2, 10) == OW_OK)
		{
			cntr++;
		...
		if (OW_Send(OW_SEND_RESET,
				(uint8_t *) "\x55\x28\xd8\x2a\xbf\x03\x00\x00\xd1\xbe\xff\xff",
				12, DS_OUT[1].buf, 2, 10) == OW_OK)
		{
			cntr++;
		...
		if (OW_Send(OW_SEND_RESET,
				(uint8_t *) "\x55\x28\x0b\x51\xbf\x03\x00\x00\x51\xbe\xff\xff",
				12, DS_OUT[2].buf, 2, 10) == OW_OK)
		{
			cntr++;
		...
		if (OW_Send(OW_SEND_RESET,
				(uint8_t *) "\x55\x28\x97\xbb\xd5\x03\x00\x00\x71\xbe\xff\xff",
				12, DS_OUT[3].buf, 2, 10) == OW_OK)
		{
			cntr++;
		...
		{
			chSysLock();
			Core_DS18B20.current_value = (DS_OUT[0].temp + DS_OUT[1].temp
					+ DS_OUT[2].temp + DS_OUT[3].temp) / cntr;
			chSysUnlock();
			if ((old_temp != 0xffff)
					&& (abs(Core_DS18B20.current_value - old_temp) > (10 << 2)))
			{
				chSysLock();
...
{
#if DS18B20_PRESENT
	chThdCreateStatic(waDS18B20, sizeof(waDS18B20), HIGHPRIO, DS18B20,
			(void*) (uint32_t) id);
#endif
}
