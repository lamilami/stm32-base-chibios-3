      ...
      for (num = 0; num < 3; num++) {
        ...
        if ((shift) || (old_range[val] != range[val])) {
          if (Curr_Elem->val[val][num] > -50) {
            if ((Vals_prev[0].val[val][num] > -50)
              ...
              gdispDrawLine(
                  x + val * (width / 2) + x_start,
                  height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT
                      - (Vals_prev[0].val[val][num] - old_min[val]) * old_range[val],
                  x + 1 + val * (width / 2) + x_start,
                  height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - (Curr_Elem->val[val][num] - old_min[val]) * old_range[val],
                  BG_Color[val]);
            }
        ...
        if (Curr_Elem->val[val][num] > -50) {
          if ((Vals_prev[1].val[val][num] > -50)
            ...
            gdispDrawLine(
                x + val * (width / 2) + x_start,
                height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - (Vals_prev[1].val[val][num] - min[val]) * range[val],
                x + 1 + val * (width / 2) + x_start,
                height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - (Curr_Elem->val[val][num] - min[val]) * range[val],
                Text_Color[num]);
          }
...
//                  GRAPH_COLOR);
    ...
    gdispFillStringBox((width / 2) * val + 10 + D1_VALS + H1_VALS + M5_VALS + 3, height - NUMBERS_HEIGHT - 17, S10_VALS,
                       12, "5m", font10, GRAPH_COLOR, BG_Color[val], justifyCenter);
...
//                  GRAPH_COLOR);
    ...
    gdispFillStringBox((width / 2) * val + 10 + D1_VALS + H1_VALS + 2, height - NUMBERS_HEIGHT - 17, M5_VALS, 12, "3h",
                       font10, GRAPH_COLOR, BG_Color[val], justifyCenter);
    ...
    gdispFillStringBox((width / 2) * val + 10 + D1_VALS + 1, height - NUMBERS_HEIGHT - 17, H1_VALS, 12, "1d", font10,
                       GRAPH_COLOR, BG_Color[val], justifyCenter);
    ...
    gdispFillStringBox((width / 2) * val + 10, height - NUMBERS_HEIGHT - 17, D1_VALS, 12, "4w", font10, GRAPH_COLOR,
                       BG_Color[val], justifyCenter);
...
//    gdispClear(White);
  ...
  while (TRUE) {
    ...
    if ((DateTime.millisecond - (DateTime.millisecond % 1000)) % 2000 == 0) {
...
//      DHT_Temp_Vals.humidity = -99;
        if (Core_Module_Read(SUBMOD_ADDR(num+1), DHT11, 0, (char*)&DHT_Temp_Vals) != MSG_TIMEOUT) {
          ...
          Vals_Cur_s10.hum[num] = -99;
          Vals_Cur_s10.temp[num] = -77;
        }
      }
...
//      static uint8_t cnt_s10 = 0, cnt_m5 = 0, cnt_h1 = 0;
      ...
      for (val = 0; val < 2; val++) {
        ...
        if ((old_min[val] != min[val]) || (old_max[val] != max[val])) {
          ...
          gdispFillStringBox((width / 2) * (val + 1) - 21, height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - 5, 15, 10, buf,
                             font12, GRAPH_COLOR, BG_Color[val], justifyRight);
          if (min[val] != max[val])
            sprintf_(buf, "%02i", max[val]);
          else
            sprintf_(buf, "    ", max[val]);
          gdispFillStringBox((width / 2) * (val + 1) - 21, height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - 5 - GRAPH_HEIGHT,
                             15, 10, buf, font12, GRAPH_COLOR, BG_Color[val], justifyRight);
        }
      }
...
//      }
      ...
      if ((old_max[0] != max[0]) || (old_max[1] != max[1]) || (old_min[0] != min[0]) || (old_min[1] != min[1])) {
        ...
        gdispFillArea(10 + 1, height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - GRAPH_HEIGHT, D1_VALS, GRAPH_HEIGHT + 1,
                      TEMP_BG_COLOR);
        gdispFillArea(10 + 1 + (width / 2), height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - GRAPH_HEIGHT, D1_VALS,
                      GRAPH_HEIGHT + 1, HUM_BG_COLOR);
        Redraw_Graph(10, D1_VALS, &old_min[0], &min[0], &old_range[0], &range[0], &Graph_Data.d1[0], &Vals_Cur_d1,
                     FALSE);

        gdispFillArea(10 + D1_VALS + 2, height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - GRAPH_HEIGHT, H1_VALS,
                      GRAPH_HEIGHT + 1, TEMP_BG_COLOR);
        gdispFillArea(10 + D1_VALS + 2 + (width / 2), height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - GRAPH_HEIGHT, H1_VALS,
                      GRAPH_HEIGHT + 1, HUM_BG_COLOR);
        Redraw_Graph(10 + D1_VALS + 1, H1_VALS, &old_min[0], &min[0], &old_range[0], &range[0], &Graph_Data.h1[0],
                     &Vals_Cur_h1, FALSE);
        gdispFillArea(10 + D1_VALS + H1_VALS + 3, height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - GRAPH_HEIGHT, M5_VALS,
                      GRAPH_HEIGHT + 1, TEMP_BG_COLOR);
        gdispFillArea(10 + D1_VALS + H1_VALS + 3 + (width / 2), height - NUMBERS_HEIGHT - GRAPH_Y_SHIFT - GRAPH_HEIGHT,
                      M5_VALS, GRAPH_HEIGHT + 1, HUM_BG_COLOR);
        Redraw_Graph(10 + D1_VALS + H1_VALS + 2, M5_VALS, &old_min[0], &min[0], &old_range[0], &range[0],
                     &Graph_Data.m5[0], &Vals_Cur_m5, FALSE);
        Redraw_Graph(10 + D1_VALS + H1_VALS + M5_VALS + 3, S10_VALS, &old_min[0], &min[0], &old_range[0], &range[0],
...
#if ILI9341_PRESENT
//  ILI9341_Init();
  chThdCreateStatic(waILI9341, sizeof(waILI9341), LOWPRIO, ILI9341, NULL);
