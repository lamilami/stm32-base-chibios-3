...
//	Core_FloorHeater.next = NULL;
  ...
  Core_FloorHeater.inner_values[0] = &Inner_Val_FH;
  Core_FloorHeater.ival_size = sizeof(FloorHeater_Inner_Val);
  Core_FloorHeater.ival_rw_size = sizeof(FloorHeater_Inner_Val_RW);

  Inner_Val_FH[0].RW.iGain = 2;
  Inner_Val_FH[0].RW.pGain = 4;
  Inner_Val_FH[0].RW.iState = 0;
  Inner_Val_FH[0].RW.iMax = 25;
  Inner_Val_FH[0].RW.iMin = -2;
  Inner_Val_FH[0].RW.Desired_Temp = 0;
  Inner_Val_FH[0].RW.Get_Temp_Callback = NULL;
  Inner_Val_FH[0].RW.MaxPower = 100;
...
//	pwmEnableChannel(&PWMD1, 2, PWM_PERCENTAGE_TO_WIDTH(&PWMD1, 9000));   // 10% duty cycle
  while (TRUE) {
    ...
    eventmask_t evt = chEvtWaitOneTimeout(ALL_EVENTS, S2ST(Core_FloorHeater.Auto_Update_Sec));
